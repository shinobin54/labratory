...
/*Fp,EFp*/
mpz_t Fp_p,BLS_r,BLS_t,BLS_s,BLS_x;
Fp Fp_unity,Fp_zero;
Fp BLS_b,BLS_a;
/*Fp2,EFp2*/
Fp Fp2_i,Fp2_i2;
...
void EFp2_sub(EFp2 *rop,EFp2 *op1,EFp2 *op2){
  ...
  EFp2_init(&inv);
  EFp2_inv(&inv,op2);
  EFp2_ADD(rop,op1,&inv);
  EFp2_clear(&inv);
}
void EFp2_inv(EFp2 *rop,EFp2 *op){
  Fp2_set(&rop->x,&op->x);
  Fp2_sub(&rop->y,&Fp2_zero,&op->y);
...
void example(){
  ...
  mpz_set_ui(Fp_p,139);
  mpz_sub_ui(BLS_r,Fp_p,1);
  mpz_sub_ui(BLS_s,Fp_p,1);
  mpz_set_ui(BLS_t,0);
  Fp_sub_ui(&BLS_a,&Fp_zero,13);
  Fp_sub_ui(&BLS_b,&Fp_zero,7);
...
void Fp2paring(Fp2 *rop,EFp2 *p,EFp2 *q){
  ...
  mpz_init(temp);
  //最終べき
  mpz_sub_ui(temp,Fp2_p,1);
  mpz_divexact(temp,temp,BLS_r);
  Fp2_Miller(rop,BLS_r,p,q);
...
void EFp2_set_Q(EFp2 *Q){
  ...
  EFp2_init(&tmp);
  EFp2_random(&tmp);
  Fp2_pow(&Q->x,&tmp.x,Fp_p);/*frobenius*/
  Fp2_pow(&Q->y,&tmp.y,Fp_p);/*frobenius*/
  EFp2_sub(Q,Q,&tmp);
}
void inversiontest(){
  EFp2 P,Q;
  ...
  */
  ...
  /*Q=(59,-54)*/
  Fp_set_ui(&P.x.x0,59);
  Fp_sub_ui(&P.y.x0,&Fp_zero,54);

  Fp2paring(&rop,&P,&Q);
  Fp2_print(&rop);
}
/*
void paring_inversion(EFp2 *Q,Fp2 *v,EFp12 *A){
  Fp2 u;
  Fp x1,x2;
  mpz_t tmp;
  Fp2_init(&u);
  ...
  mpz_init(tmp);
  //1
  mpz_add_ui(&tmp,&Fp_p,1);
  mpz_divexact_ui(&tmp,&tmp,2);
  Fp2_pow_ui(&u,v,tmp);
  //2
  if(Fp_cmp(&u.x1,Fp_zero)==0) return;//return nil
  //3
  Fp_add(&x1,,)
...
void paringtest(){
  ...
  EFp12_SCM(&r,&r,temp);
  Fp12_pow(&(EFp12_temp.x),&(r.x),Fp_p);
  Fp12_pow(&(EFp12_temp.y),&(r.y),Fp_p);
...
int main(){
  ...
  EFp2_random(&EFp2_2);
  EFp2_DBL(&EFp2_1,&EFp2_2);
  EFp2_DBL(&EFp2_1,&EFp2_1);
  EFp2_print(&EFp2_1);

  EFp2_ADD(&EFp2_1,&EFp2_2,&EFp2_2);
  EFp2_ADD(&EFp2_1,&EFp2_1,&EFp2_2);
  EFp2_ADD(&EFp2_1,&EFp2_1,&EFp2_2);
  EFp2_print(&EFp2_1);
...
void BLS_make(){
  ...
  mpz_sub(BLS_r, temp, temp2);
  mpz_add_ui(BLS_r, BLS_r, 1);

  mpz_sub_ui(temp, BLS_x, 1);
  mpz_pow_ui(temp, temp, 2);
  mpz_mul(temp, BLS_r, temp);
  mpz_set_ui(temp2, 3);
  if(mpz_divisible_p(temp, temp2)!=0){
    mpz_cdiv_q(Fp_p, temp, temp2);
    mpz_add(Fp_p, Fp_p, BLS_x);
  }else{
    mpz_set_ui(Fp_p,1);
  }
  if( mpz_probab_prime_p(Fp_p,50)==0 && mpz_probab_prime_p(BLS_r,50)==0){
    printf("x:error\n");
    //return 0;
  }
  //BLS_tの設定
...
void Fp12_Frobenius(Fp12 *rop,Fp12 *op){
  ...
  Fp_pow(&(rop->x1.x0.x0),&(op->x1.x0.x0),Fp_p);
  //w7
  //Fp_mul(&(rop->x1.x0.x1),&(op->x1.x0.x1),&c12);
  //Fp_neg(&(rop->x1.x0.x1),&(rop->x1.x0.x1));
  Fp_pow(&(rop->x1.x0.x1),&(op->x1.x0.x1),Fp_p);
  //w8
  ...
  Fp_pow(&(rop->x1.x2.x0),&(op->x1.x2.x0),Fp_p);
  //w5
  //Fp_mul(&(rop->x1.x2.x1),&(op->x1.x2.x1),&c5);
  //Fp_neg(&(rop->x1.x2.x1),&(rop->x1.x2.x1));
  Fp_pow(&(rop->x1.x2.x1),&(op->x1.x2.x1),Fp_p);
}
